...
import cat.itacademy.barcelonactiva.fernandezvidal.alejandro.s05.t02.n01.S05T02N01FernandezVidalAlejandro.security.JwtEntryPoint;
import cat.itacademy.barcelonactiva.fernandezvidal.alejandro.s05.t02.n01.S05T02N01FernandezVidalAlejandro.security.TokenUtils;
...
public class JugadorServiceImpl implements IJugadorService {
	...
	public JugadorDTO añadirJugador(JugadorDTO jugadorDTO/* AuthCredentials authCredentials */) {
		...
		if (jugadorDTO.getNombre().equals("ANÒNIM")
				/* authCredentials.getNombre().equals("ANÒNIM") */ || !jugadorRepository
						.existsByNombre(jugadorDTO.getNombre()/* authCredentials.getNombre() */)) {

			JugadorEntity jugadorEntity = new JugadorEntity();
			// JugadorDTO jugadorDTO = new JugadorDTO();
			jugadorEntity.setNombre(jugadorDTO.getNombre());
			// jugadorEntity.setNombre(authCredentials.getNombre());
			// jugadorDTO.setNombre(authCredentials.getNombre());
			jugadorEntity.setLocalDate(LocalDate.now());
			jugadorEntity.setPassword(new BCryptPasswordEncoder().encode(jugadorDTO.getPassword()));
	...
	public boolean existsJugadorById(ObjectId Id) {
		return jugadorRepository.existsById(Id);
	}
	
	public boolean validarJugador(ObjectId id, HttpServletRequest request) {
		boolean resultado = false;
		String nombreById = jugadorRepository.findById(id).get().getNombre();
		String token = jwtEntryPoint.getToken(request);
		String nombreByToken = TokenUtils.getUsernameFromToken(token);
		if(nombreById.equalsIgnoreCase(nombreByToken))
			resultado = true;
		return resultado;
	}
}