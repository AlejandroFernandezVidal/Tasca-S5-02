...
public class JugadorController {
	...
	public ResponseEntity<?> crearJugador(@RequestBody JugadorDTO jugadorDTO) {
		...
		if (jugadorDTOGuardar != null) {
			...
			responseEntity = ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Ya hay un usuario con nombre " + jugadorDTO.getNombre());
		}
	...
	public ResponseEntity<?> cambiarNombreJugador(@PathVariable ObjectId id, @RequestBody JugadorDTO jugadorDTO,
		...
		ResponseEntity<?> responseEntity;
		boolean autorizado = jugadorServiceImpl.validarJugador(id, request);
		if (autorizado) {
			boolean jugadorBuscar = jugadorServiceImpl.existsJugadorById(id);
			if (jugadorBuscar) {
				...
				responseEntity = ResponseEntity.status(HttpStatus.NOT_FOUND).body("Error con el ID indicado.");
			}
		} else {
			responseEntity = ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Token no valido para usuario indicado.");
		}
		return responseEntity;
	}

	@PostMapping("/players/{id}/games/")
	public ResponseEntity<?> realizarTiradaJugador(@PathVariable ObjectId id, HttpServletRequest request) {
		ResponseEntity<?> responseEntity;
		boolean autorizado = jugadorServiceImpl.validarJugador(id, request);
		if (autorizado) {
	...
	public ResponseEntity<?> eliminarTiradasJugador(@PathVariable ObjectId id, HttpServletRequest request) {
		ResponseEntity<?> responseEntity;
		boolean autorizado = jugadorServiceImpl.validarJugador(id, request);
		if (autorizado) {
	...
	public ResponseEntity<?> obtenerListadoTiradas(@PathVariable ObjectId id, HttpServletRequest request) {
		ResponseEntity<?> responseEntity;
		boolean autorizado = jugadorServiceImpl.validarJugador(id, request);
		if (autorizado) {
